...
RUN cd /opt && \
    ...
    # code) uses Symfony DomCrawler + its CSS selector filter, but both
    # symfony/dom-crawler and symfony/css-selector are only DEV dependencies
    # of mautic/core — absent from any create-project install, so saving ANY
    # email containing links 500s. Pull both in explicitly.
    COMPOSER_ALLOW_SUPERUSER=1 composer --working-dir=/opt/mautic require symfony/dom-crawler symfony/css-selector --no-interaction && \
